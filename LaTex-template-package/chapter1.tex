...
A type is defined by the operations it supports and their sequential specification. An object is an implementation of a type.

Type $dynamic\_task\_alloc$ supports two operation $DoTask($ $)$ and $InsertTask( \ell )$, where $\ell$ is a task identifier. The $DoTask($ $)$ operation performs an available task and returns the index of that task. If there is no available task, then it returns $\perp$. The $InsertTask( \ell )$ operation insert the task $\ell$ to be performed and returns $success$ if $\ell$ has been inserted successfully.

The sequential specification of the type $dynamic\_task\_alloc$ is presented as follows: (1) (Validity) If a $DoTask($ $)$ returns $\ell$, then before that, an $InsertTask( \ell )$ was executed and returned success\\
(2) (Uniqueness) For each task $\ell$, at most one $InsertTask( \ell )$ returns $success$ and at most one $DoTask($ $)$ returns $\ell$.
