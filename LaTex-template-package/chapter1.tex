...
The computational model we consider is the standard asynchronous shared memory model with $n$ processes $1, 2, ..., n$, where up to $n-1$ processes may fail by crashing. The processes communicate with each other via shared compare-and-swap objects. Each process executes its program by taking steps which are defined to be the executions of all local computations followed by an operation on a shared register.

A CAS object $v$ is a shared memory object stores a value from some set and supports two atomic synchronization operations $v.read()$ and $v.CAS((x,y), (a, b))$. Operation $v.read()$ returns the current value of the object and leaves the value unchanged. A $v.CAS((x,y), (a, b))$ takes the memory locations and writes new value into it only if it matches pre-supplied ``expected" values $(x, y)$, i.e, if current value of $v$ are exactly $(x, y)$, then the operation $v.CAS((x,y), (a, b))$ succeeds, and the value of $v$ is changed to be $(a, b)$ and $true$ is returned. Otherwise, the operation fails, the current value of $v$ remains unchanged and $false$ is returned.

Each process has a unique identifier $i$ which is from an unbounded namespace and has a local random number generator to perform coin flip operations. In the following discussion, we use $random(s)$ to return a value distributed uniformly at random from the set $\{0, 1, 2,..., s-1\}$.

We analyze the algorithm under the assumption of a strong adaptive adversary. At any point of time, it can see the entire history and know the states of all processes.
Base on such history, it decides which process takes the next step.
...
A type is defined by the operations it supports and their sequential specification. An object is an implementation of a type. Our type $dynamic\_task\_alloc$ supports two operation $DoTask()$ and $InsertTask( \ell )$, where $\ell$ is a task identifier. The $DoTask$ operation performs an available task and returns the index of that task. If there is no available task, then it returns $\perp$. The $InsertTask( \ell )$ operation insert the task $\ell$ to be performed and returns $success$ if $\ell$ has been inserted successfully.

The sequential specification of the type $dynamic\_task\_alloc$ is presented as follows:\\
\noindent $\bullet$ Validity: If a $DoTask($ $)$ returns $\ell$, then before that, an $InsertTask( \ell )$ was executed and returned success\\
\noindent $\bullet$ Uniqueness: For each task $\ell$, at most one $InsertTask( \ell )$ returns $success$ and at most one $DoTask($ $)$ returns $\ell$.

For the implementations, we would like every inserted task $\ell$ will finally be performed which is a desired progress property of implementations of type $dynamic\_task\_alloc$.
