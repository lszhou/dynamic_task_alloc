...
\textbf{Read-Write Register}. An object that supports only \texttt{read()} and \texttt{write(x)} operations is called
...
Performing a \emph{task} can be be any computation which can be performed by a single
process in constant time and the tasks are assumed to satisfy the following three
properties \cite{georgiou2007all}:

\begin{itemize}
  \item \emph{Similarity}: The computations consume equal or comparable resources
  \item \emph{Independence}: One computation is independent of any of the others.
  \item \emph{Idempotence}: Tasks admit at-least-once execution semantics.
\end{itemize}
