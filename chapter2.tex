...
The task $\ell$ can be performed atomically by a process by calling a special
\texttt{TryTask(}$M$\texttt{)} operation where $M$ is the location task $\ell$
associated with.

Out of several processes calling \texttt{TryTask(}$M$\texttt{)} concurrently,
only one receives \emph{success} and the identifier $\ell$ of the
task, whereas all the others receive a \emph{failure} notification.
This ensures that only a single process may actually perform
the task.

\textbf{Operation PutTask}.
The task $\ell$ can be associated with a memory location $M$ in the data structure
atomically by a process by calling a special \texttt{PutTask(}$M,\ell$\texttt{)} operation.

If location $M$ is not associated with any other task, out of several processes calling
\texttt{PutTask(}$M,\ell$\texttt{)} concurrently,
only one of them receives \emph{success}, whereas all the others receive \emph{failure}.

If location $M$ is
already associated with another task $\ell'$, then $failure$ is returned for all \texttt{PutTask(}$M,\ell$\texttt{)} calls.

\subsection{The Type DTA}

Our type DTA supports two types of operations. The \texttt{DoTask()} operation performes a new task
and returns the identifier of that task, while the \texttt{InsertTask(}$\ell$\texttt{)} operation inserts task
$\ell$ to the data structure to be performed.

\textbf{Operation DoTask()}.
The aim of operation \texttt{DoTask()} is to find a task in the data structure and perform the task by calling
atomic operation \texttt{TryTask(}$M$\texttt{)}.

Out of several processes
...
task identifier $\ell$ will be returned by \texttt{DoTask()}. If there is no task in the data structure,
then \texttt{DoTask()} returns $\perp$.

A task is said to be \emph{performed} by a process if the process has completed a \texttt{DoTask()} call.

\textbf{Operation InsertTask($\ell$)}.
The goal of \texttt{InsertTask(}$\ell$\texttt{)} operation is to find a location $M$ in the shared data structure and
insert task $\ell$ by calling atomic operation \texttt{PutTask(}$M,\ell$\texttt{)} which is assumed to exist.

Every \texttt{InsertTask(}$\ell$\texttt{)} operation may perform several \texttt{PutTask} operations.
However, only one of them will succeed. Once one \texttt{PutTask(}$M,\ell$\texttt{)} succeeds, then $success$
is returned by \texttt{InsertTask(}$\ell$\texttt{)} operation.

We say task $\ell$ is associated with a memory location or inserted into the data structure or task $\ell$
is \emph{available} to perform if a process has completed the \texttt{InsertTask(}$\ell$\texttt{)} call,
but task $\ell$ has not been performed yet.
