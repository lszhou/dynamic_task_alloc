...
A vector $\overrightarrow{\rm c} = (c_1, c_2, c_3, ...) \in \Omega^{\infty}$ is called a \emph{coin-flip vector}.
An history $H$ is said to \emph{observe} the coin-flip vector $\overrightarrow{\rm c}$ if
...
A \emph{schedule} with \emph{lenght} $k+1$ is represented by a sequence of process ids
$\mathcal{P} = (p_0, p_1, p_2,..., p_k)$.
A history $H$ is said to \emph{observe} schedule $\mathcal{P}$ if the number of events in $H$ is $k$,
and for $i \in (0, k)$, the $i$-th event is applied by process $p_i$.
