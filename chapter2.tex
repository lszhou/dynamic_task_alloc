...
$\ell$ in the data structure and then perform task $\ell$ by calling atomic operation \texttt{TryTask(}$M$\texttt{)}.
...
\texttt{PutTask} operations succeeds. Otherwise, \texttt{InsertTask(}$\ell$\texttt{)} never terminates and keeps
calling \texttt{PutTask} operations repeatedly.

% However, only one of them will succeed. (this is the progress property, should not be descirped here)
