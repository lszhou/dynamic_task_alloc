...
Our type DTA supports two types of operations, \texttt{DoTask()} and
...
atomic operation \texttt{TryTask(}$M$\texttt{)} which is assumed to exist, where $M$ is the place in the data structure where
task $ell$ located. i.e, task $\ell$ located at $M$ can be performed atomically by a process by calling \texttt{TryTask(}$M$\texttt{)}.
...
Task $\ell$ can be inserted to location $M$ atomically by calling \texttt{PutTask(}$M,\ell$\texttt{)}.
\texttt{PutTask(}$M,\ell$\texttt{)} returns $success$ if task $\ell$ is inserted to $M$,
and returns $failure$ if there is a task has been inserted to location $M$,
i.e, there is a successful \texttt{PutTask(}$M,\ell'$\texttt{)} operation
...
A task $\ell$ is said \emph{available} or \emph{inserted} by a process if the process has
completed the \texttt{InsertTask(}$\ell$\texttt{)} call, but task $\ell$ has not been performed yet.
