...
only the first one receives \emph{success} and the identifier $\ell$ of the
...
only the first one receives notification \emph{success}, whereas all the others receive \emph{failure}.

If location $M$ is
already associated with another task $\ell'$, then $failure$ is returned by all \texttt{PutTask(}$M,\ell$\texttt{)} calls.

\subsection{The Type DTA}

The type DTA supports two types of operations. The \texttt{DoTask()} operation performes a new task
and returns the identifier of that task, while the \texttt{InsertTask(}$\ell$\texttt{)} operation associates task
$\ell$ with a location in the data structure to be performed.
...
arguments. However, only one of them will succeed. Once one \texttt{TryTask(}$M$\texttt{)} succeeds, then
...
associates task $\ell$ with $M$ by calling atomic operation \texttt{PutTask(}$M,\ell$\texttt{)}.

Every \texttt{InsertTask(}$\ell$\texttt{)} operation may perform several \texttt{PutTask} operations with
different locations as the arguments. However, only one of them will succeed.
Once one \texttt{PutTask(}$M,\ell$\texttt{)} succeeds, then $success$
